/**
 * Gestión de invitaciones (admin+) — email-invitations, Fase 6.
 *
 ...
 *
 * El backend es la autoridad de permisos (require_min_role(ADMIN)); el guard
 * client-side de abajo sólo evita renderizar una sección que la API va a
 * negar igual (mismo criterio que la página de beta testers).
 */
...
import type { Invitation, InvitationStatus, InvitationWithToken, UserRole } from '@/lib/types';
...
export default function InvitationsAdminPage() {
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      invitation = await createInvitation(email, role);
    } catch (err: unknown) {
  ...
  // Gate client-side: denegación sin renderizar datos. El enforcement real
  // (401/403) es del backend — esto es no mostrar puertas que la API cierra.
  if (user && !isAdmin) {
    return (
      <div className="p-6">
        <Card>
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        <h1 className="font-heading text-2xl font-bold tracking-tight">Invitaciones</h1>
        <p className="text-sm text-muted-foreground">
          El acceso a la plataforma es solo por invitación: creá una y el link de alta viaja por
          email al invitado.
        </p>
      </div>
